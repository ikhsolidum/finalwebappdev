...
if(isset($_REQUEST['login_submit'])){
	$login = $user->check_login($_REQUEST['useremail'],md5($_REQUEST['password']));
	if($login){
		header("location: http://localhost/webappdev/index.php?page=home");
	}else{
		?>
	...
	$result = $user->register($_REQUEST['name'], $_REQUEST['email'], md5($_REQUEST['password']));
	if ($result === true) {
		header("location: http://localhost/webappdev/index.php?page=home");
	} else {
		?>
